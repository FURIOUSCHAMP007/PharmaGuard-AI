...
import { ClinicalStressTestModule } from '../ClinicalStressTestModule';
import { ClinicalGoalTracker } from '../ClinicalGoalTracker';
import { ClinicalNarrativeSummary } from '../ClinicalNarrativeSummary';
import { MedicationAdministrationTimeline } from '../MedicationAdministrationTimeline';
import { MedicationAdherenceScore } from '../MedicationAdherenceScore';
import { LabResultInsight } from '../LabResultInsight';
import { VitalsCorrelationScatterPlot } from '../VitalsCorrelationScatterPlot';
import { GNNGraphVisualizer } from '../GNNGraphVisualizer';
import { EmergencyOverridePanel } from '../EmergencyOverridePanel';
...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'vitals_predictive' | 'ai_knowledge' | 'regimen_schedule' | 'all'>('overview');
  ...
  const handleDownloadVitalsCSV = () => {
    ...
    const baseHR = activePatient.vitals.heartRate || 74;
    const baseSys = activePatient.vitals.bpSystolic || 128;
    const baseDia = activePatient.vitals.bpDiastolic || 82;
    const baseSpo2 = 97;
    const baseQtc = activePatient.vitals.qtcIntervalMs || 440;
    ...
    for (let i = 24; i >= 0; i--) {
      ...
      const hr = Math.min(130, Math.max(55, Math.round(baseHR * (0.9 + Math.sin(i * 0.4) * 0.12))));
      const sys = Math.min(170, Math.max(95, Math.round(baseSys * (0.92 + Math.cos(i * 0.35) * 0.1))));
      const dia = Math.min(105, Math.max(60, Math.round(baseDia * (0.92 + Math.cos(i * 0.35) * 0.08))));
      const spo2Val = Math.min(100, Math.max(90, Math.round(baseSpo2 - (i === 14 ? 3 : 0))));
      const qtcVal = Math.min(520, Math.max(390, Math.round(baseQtc - 20 + (i <= 14 && i >= 4 ? 35 : 0))));
    ...
    link.setAttribute('download', `${activePatient.name.replace(/\s+/g, '_')}_Vitals_Data.csv`);
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-10">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex flex-wrap items-center justify-between gap-4 text-white shadow-md">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button
            ...
            className="px-3.5 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 font-mono font-bold text-xs shadow-md flex items-center gap-2 transition-all cursor-pointer"
      ...
      <div className="flex items-center justify-between bg-slate-900 border border-slate-800 rounded-xl p-1.5 overflow-x-auto gap-1 text-xs font-semibold shadow-inner">
        <button
        ...
        <button
          onClick={() => setActiveTab('vitals_predictive')}
          className={`flex items-center gap-2 px-3.5 py-2 rounded-lg transition-all cursor-pointer whitespace-nowrap ${
            activeTab === 'vitals_predictive'
              ? 'bg-indigo-600 text-white font-bold shadow-md'
              : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/60'
          }`}
        >
          <TrendingUp className="w-4 h-4 text-cyan-400" />
          <span>Vitals & Stress Tests</span>
        </button>
        ...
        >
          ...
          <span>AI Narrative & Knowledge Graph</span>
        </button>

        <button
          onClick={() => setActiveTab('regimen_schedule')}
          className={`flex items-center gap-2 px-3.5 py-2 rounded-lg transition-all cursor-pointer whitespace-nowrap ${
            activeTab === 'regimen_schedule'
              ? 'bg-indigo-600 text-white font-bold shadow-md'
              : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/60'
          }`}
        >
          <Pill className="w-4 h-4 text-purple-400" />
          <span>Regimen & Schedule</span>
        </button>
      ...
      >
        <div className="space-y-6">
          {/* Executive Summary Cards - Always visible on Overview and All */}
          ...
          {/* Vitals & Predictive Section */}
          {(activeTab === 'vitals_predictive' || activeTab === 'all') && (
            <div className="space-y-6">
              ...
              {/* 24-Hour Patient Vital Signs Trend Visualization */}
              <PatientVitalsTrend patient={activePatient} />

              {/* Physiological Vitals Correlation & Outlier Scatter Plot */}
              <VitalsCorrelationScatterPlot patient={activePatient} />

              {/* Predictive Clinical Stress Test (+50% Dosage Escalation Simulator) */}
              <ClinicalStressTestModule patient={activePatient} />
            </div>
          )}

          {/* AI Narrative & Knowledge Graph Section */}
          {(activeTab === 'ai_knowledge' || activeTab === 'all') && (
            <div className="space-y-6">
              {/* AI LLM Clinical Narrative Summary (Plain-Language & Specialist View) */}
              <ClinicalNarrativeSummary patient={activePatient} />

              {/* Knowledge Graph Lab Result Insights (Plain-Language & Biomedical Node Traversal) */}
              <LabResultInsight patient={activePatient} />
          ...
          {/* Regimen & Schedule Section */}
          {(activeTab === 'regimen_schedule' || activeTab === 'all') && (
            <div className="space-y-6">
              {/* Patient Clinical Goal & Target Outcome Tracker */}
              <ClinicalGoalTracker patient={activePatient} />

              {/* Medication Adherence Score & PDC Compliance Gauge */}
              <MedicationAdherenceScore patient={activePatient} />

              {/* 24-Hour Medication Administration Timeline (eMAR Schedule) */}
              <MedicationAdministrationTimeline patient={activePatient} />
            </div>
          )}

          {/* Visual Analytics Overview Section (Visible on Overview and All) */}
          {(activeTab === 'overview' || activeTab === 'all') && (
          ...
          {/* Main Grid (Active Case Regimen, Interactions, Multi-Agent Console, FDA) - Visible on Overview & All */}
          {(activeTab === 'overview' || activeTab === 'all') && (
      ...
      {/* Floating Emergency Override Action Panel */}
      <EmergencyOverridePanel patient={activePatient} onNavigate={(v) => onNavigate(v as ViewId)} />
    </div>
  );
};
